...
public class FilliereController {
	...
	private Filier filier= new Filier();;
...
private void addWidgetsToGridPane(GridPane gridPane) {
    ...
    try {
        ...
        if (!filiers.isEmpty()) {
            for (Filier filier : filiers) {
                ...
                String imagePath = Paths.get("C:/Users/shop souf/eclipse-workspace/Java_SchoolManagement/src/assets/logoibnzohr.png").toUri().toString();
                Pane widget1 = loader.loadFilliereWidget(
                    filier.getTitel(),
                    filier.getShortName(),
                    new Image(imagePath) // Corrected Image path
                );
    ...
    public void saveFilliereToDB() {
    	...
    	try {
        	filier.setTitel(filliereNameTF.getText());
        	filier.setShortName(filliereShortNameTF.getText());
